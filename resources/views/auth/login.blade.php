...
<head>
    <meta charset="UTF-8">
    <title>Iniciar sesión - Emprendimientos Sucre</title>
...
<body class="min-h-screen flex items-center justify-center p-4 py-12 relative bg-slate-50 font-['Outfit']">
        ...
        <div class="bg-white/85 backdrop-blur-xl border border-white/50 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.1)] rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 md:p-12 relative overflow-hidden group">
            ...
            <div class="text-center mb-8 sm:mb-10 relative">
                <div class="inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-[1.5rem] sm:rounded-[2rem] bg-gradient-to-tr from-indigo-600 via-violet-600 to-fuchsia-500 text-white shadow-2xl shadow-indigo-500/30 mb-4 sm:mb-6 transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 ring-4 ring-white/50">
                     <span class="material-icons-round text-3xl sm:text-4xl">face</span>
                </div>
                <h1 class="text-2xl sm:text-3xl font-black text-slate-800 tracking-tight mb-2">¡Bienvenido!</h1>
                <p class="text-slate-500 text-xs sm:text-sm font-medium px-4">Nos alegra verte de nuevo. <br> Ingresa para continuar.</p>
            </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center justify-between ml-1">
                        ...
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-800 hover:underline transition-colors">
                                ¿Olvidaste la clave?
                            </a>
                        @endif
                    </div>
            ...
            <div class="mt-8 text-center">
                @if (Route::has('register'))
                    <a href="{{ route('register') }}" class="inline-block px-6 py-2 rounded-xl text-sm font-bold text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 transition-all border border-transparent hover:border-indigo-100">
                       ¿No tienes cuenta? <span class="text-indigo-600">Regístrate aquí</span>
                    </a>
                @endif
            </div>
    ...
    @if (session('swal_logout'))
        <script>
            Swal.fire({
                icon: 'success',
                title: '¡Hasta luego!',
                text: "{{ session('swal_logout') }}",
